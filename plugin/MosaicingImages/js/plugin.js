...
async function loadConfig() {
	...
	try {
		...
		config = { // 設定読み込み失敗時のデフォルト値
			...
			baseMosaicModel: "ntd11_anime_nsfw_segm_v1_all.pt:0.2", // 基本のモザイクモデル
...
async function processImages(currentConfig, uiTargetFolder, uiTargetTags) {
	...
	for (const item of items) {
		...
		// 2. タグフィルタリングのスキップ判定 (configで有効になっている場合のみ)
		//    - UIでタグが指定されている場合 => アイテムにタグがなければスキップ
		//    - UIでタグが指定されていない場合 => この条件ではスキップしない
		if (currentConfig.tagFilter && uiTargetTags.length > 0 && item.tags.length === 0) {
			console.log(`アイテムをスキップ (タグフィルタ有効 & UIタグ指定あり & アイテムタグなし): ${item.name} (ID: ${item.id})`);
			continue; // このアイテムの処理をスキップ
		}
		// 3. タグフィルタリングのスキップ判定 (configで有効になっている場合のみ)
		//    - UIでタグが指定されていない場合 => アイテムにタグがなければスキップ (これは元のロジックにあったが、UI空なら全処理の意図に反するためコメントアウト)
		// if (currentConfig.tagFilter && uiTargetTags.length === 0 && item.tags.length === 0) {
		//     console.log(`アイテムをスキップ (タグフィルタ有効 & UIタグ指定なし & アイテムタグなし): ${item.name} (ID: ${item.id})`);
		//     continue;
		// }
		...
		try {
			...
			await new Promise((resolve, reject) => {
				// pythonPath: Python実行ファイル, args: 引数リスト
				// encoding: 'buffer' (標準出力/エラーをバッファで受け取る)
				// cwd: Pythonスクリプトのディレクトリを作業ディレクトリにする (相対パス解決のため)
				execFile(pythonPath, args, { encoding: 'buffer', cwd: path.dirname(scriptPath) }, (error, stdout, stderr) => {
					if (error) { // スクリプト実行自体でエラーが発生した場合
						console.error(`Pythonスクリプト実行エラー (${filePath}):`, error);
						console.error(`Stderr: ${stderr.toString()}`); // 標準エラー出力も表示
						reject({ error, stdout, stderr }); // Promiseをreject
					} else { // 正常終了した場合
						console.log(`Stdout (${filePath}): ${stdout.toString()}`); // 標準出力を表示
						resolve({ stdout, stderr }); // Promiseをresolve
					}
				});
			});

			// --- モザイク処理後のファイル操作 ---
			const originalExt = path.extname(filePath); // 元ファイルの拡張子を取得 (例: '.png')
			// モザイク処理済みファイル名を生成 (元ファイル名 + '_mosaic' + 拡張子)
			const mosaicFileName = path.basename(filePath, originalExt) + '_mosaic' + originalExt;
			// モザイク処理済みファイルのフルパスを生成
			const mosaicFilePath = path.join(path.dirname(filePath), mosaicFileName);

			// モザイクファイルが実際に生成されたか確認
			try {
				await fs.access(mosaicFilePath); // ファイルアクセスを試みる (なければエラー)
			} catch {
				console.log(`モザイクファイルが見つかりません SKIP: ${mosaicFilePath}`);
				continue; // モザイクファイルがない場合は、このアイテムの以降の処理をスキップ
			}

			// オプション: 元画像を置き換える前にオリジナルを別名で保存する場合 (必要ならコメント解除)
			...
			if (replaceImage) {
				console.log(`${filePath} を ${mosaicFilePath} で置き換え`);
				await item.replaceFile(mosaicFilePath); // Eagle API を使ってアイテムのファイルを置き換え
...
eagle.onPluginShow(async () => {
	...
	if (mosaicButton) {
		...
		mosaicButton.onclick = async () => {
			...
			try {
				// 画像処理本体の関数を呼び出し
				await processImages(config, uiFolder, uiTagsArray);
			} catch (error) { // processImages内でキャッチされなかった予期せぬエラー
				console.error("モザイク処理中に予期せぬエラー:", error);
				eagle.log.error("モザイク処理中に予期せぬエラーが発生しました。");
...
async function restoreSingleItem(item) {
	...
	try {
		...
		// 6. プレビューファイル (_preview) があれば削除
		const previewFileName = path.basename(filePath, originalExt) + '_preview' + originalExt;
		const previewFilePath = path.join(path.dirname(filePath), previewFileName);
		try {
			// プレビューファイルが存在するか試す (なければ fs.access がエラーを投げる)
			await fs.access(previewFilePath);
			console.log(`  プレビューファイル "${previewFilePath}" を削除中...`);
			await fs.unlink(previewFilePath);
			console.log(`  プレビューファイル削除成功。`);
		} catch (accessOrUnlinkError) {
			// ファイルが存在しない (ENOENT) 場合はエラーではなく、単にスキップ
			if (accessOrUnlinkError.code !== 'ENOENT') {
				console.error(`  プレビューファイル "${previewFilePath}" の削除に失敗しました:`, accessOrUnlinkError);
				eagle.log.warn(`プレビューファイル "${previewFileName}" の削除に失敗しました。`);
			} else {
				console.log(`  プレビューファイル "${previewFilePath}" は存在しませんでした。`);
			}
		}
