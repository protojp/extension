...
eagle.onPluginShow(async () => {
	...
	const restoreFolderInput = document.getElementById('restore-folder-input');
	...
	if (restoreFolderInput) {
		// 復元用フォルダ入力も、とりあえずターゲットフォルダと同じ値で初期化 (仕様に応じて変更)
		restoreFolderInput.value = config.targetFolderName || "";
	} else {
	...
	if (restoreButton) {
		...
		restoreButton.onclick = () => {
			...
			// TODO: オリジナル復元ロジックをここに追加する
			eagle.log.info("オリジナル復元機能は現在実装されていません。");
		};
	} else {
